#!/bin/bash
#SBATCH --job-name=LPAZT3YH # define job name
...
#SBATCH --output=output/train_ncbi_bc5disease.o
...
litgpt finetune_lora checkpoints/TinyLlama/TinyLlama-1.1B-Chat-v1.0 \
    --data JSON \
    --data.json_path ../datasets/slm4ner/NCBI-disease-train.json \
    --out_dir output/tinyllama/ncbi \
...
litgpt finetune_lora checkpoints/TinyLlama/TinyLlama-1.1B-Chat-v1.0 \
    --data JSON \
    --data.json_path ../datasets/slm4ner/BC5CDR-disease-train.json \
    --out_dir output/tinyllama/bc5cdr-disease \
    --data.val_split_fraction 0.2 \
    --data.prompt_style 'alpaca' \
    --train.epochs 8 \
    --logger_name csv \
