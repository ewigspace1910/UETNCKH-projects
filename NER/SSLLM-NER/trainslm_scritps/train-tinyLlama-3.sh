...
#SBATCH --output=output/train_bc4che_bc5chem.o
...
litgpt finetune_lora checkpoints/TinyLlama/TinyLlama-1.1B-Chat-v1.0 \
    --data JSON \
    --data.json_path ../datasets/slm4ner/BC4CHEMD-train.json \
    --out_dir output/tinyllama/BC4CHEMD \
...
litgpt finetune_lora checkpoints/TinyLlama/TinyLlama-1.1B-Chat-v1.0 \
    --data JSON \
    --data.json_path ../datasets/slm4ner/BC5CDR-chem-train.json \
    --out_dir output/tinyllama/BC5CDR-chem \
    --data.val_split_fraction 0.2 \
    --data.prompt_style 'alpaca' \
    --train.epochs 8 \
    --logger_name csv \
