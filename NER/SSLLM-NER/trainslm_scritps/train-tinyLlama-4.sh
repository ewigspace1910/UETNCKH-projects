...
#SBATCH --output=output/train_lin_s800.o
...
litgpt finetune_lora checkpoints/TinyLlama/TinyLlama-1.1B-Chat-v1.0 \
    --data JSON \
    --data.json_path ../datasets/slm4ner/linnaeus-train.json \
    --out_dir output/tinyllama/linnaeus \
...
litgpt finetune_lora checkpoints/TinyLlama/TinyLlama-1.1B-Chat-v1.0 \
    --data JSON \
    --data.json_path ../datasets/slm4ner/s800-train.json \
    --out_dir output/tinyllama/s800 \
    --data.val_split_fraction 0.2 \
    --data.prompt_style 'alpaca' \
    --train.epochs 8 \
    --logger_name csv \
