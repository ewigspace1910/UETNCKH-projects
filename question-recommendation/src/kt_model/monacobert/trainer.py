...
def Mlm4BertTrain(r_seqs, mask_seqs):
    ...
    # mlm_r_seqs: masked r_seqs, |mlm_r_seqs| = (bs, n)
    # mlm_idx: masked idx, |mask_seqs| = (bs, n)
...
class MonaCoBERT_CTT_Trainer():
    ...
    def _train(self, train_loader, metric_name):
        ...
        for idx, data in enumerate(tqdm(train_loader)):
            ...
            mlm_r_seqs, mlm_idxs = Mlm4BertTrain(r_seqs, mask_seqs) # mlm_r_seqs: for MLM, [MASK] position get 2 / mlm_idx are index of [MASK]
            mlm_r_seqs = mlm_r_seqs.to(self.device) # |mlm_r_seqs| = (bs, n)
            mlm_idxs = mlm_idxs.to(self.device) # |mlm_idxs| = (bs, n)
            ...
            ).to(self.device) # |y_hat| = (bs, n, output_size=1)

            y_hat = y_hat.squeeze() # |y_hat| = (bs, n)

            # predictions & correct
            y_hat = torch.masked_select(y_hat, mlm_idxs) #|y_hat| = (bs * n - n_mlm_idxs)
            correct = torch.masked_select(real_seqs, mlm_idxs) #|correct| = (bs * n - n_mlm_idxs)
            loss = self.crit(y_hat, correct) # |loss| = (1)
        ...
        if metric_name == "AUC": return auc_score
        elif metric_name == "RMSE": return rmse_score
    ...
    def train(self, train_loader, valid_loader, test_loader, config, EarlyStopping, path_checkpint="checkpoint.pt"):
        ...
        # early_stopping
        early_stopping = EarlyStopping(metric_name=metric_name,best_score=best_valid_score, path=path_checkpint)
        ...
        for epoch_index in range(self.n_epochs):
            ...
            valid_score = test_score = self._test(test_loader, metric_name)
        ...
        self.model.load_state_dict(torch.load(path_checkpint))
