...
class Checkpointer:
    def __init__(
        ...
        self.logger = logger
        self.save_file = None
        self.model_name = model.module.name if isinstance(model, torch.nn.DataParallel) else model.name

    def save(self, name, ema=False, **kwargs):
        ...
        data.update(kwargs)
        if ema:
            save_file = os.path.join(self.save_dir, "{}-ema.pth".format(name))
            self.logger.info("Saving EMA checkpoint to {}".format(save_file))            
        else:
            save_file = os.path.join(self.save_dir, "{}-{}.pth".format(name, self.model_name))
            self.logger.info("Saving checkpoint of {} to {}".format(self.model_name, save_file))
            print("Saving checkpoint of {} to {}".format(self.model_name, save_file))
...
@torch.no_grad()
def copy_params(model, teacher_model):
    for param, param_m in zip(model.parameters(), teacher_model.parameters()):
